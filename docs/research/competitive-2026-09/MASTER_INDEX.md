...
Not every file listed below is on every branch. PRs #5–#16 were still OPEN/DRAFT when the handoff was written. If a path 404s locally, open the PR in §1.
...
These PRs are **not all merged**. Unique artifacts live on the listed branches. **Append, do not wipe.**
...
Handoff PR (this file’s sibling): [`GROKBOT_CURSOR_HANDOFF.md`](./GROKBOT_CURSOR_HANDOFF.md).
...
Rebuild (after Mac merge of #14 + #16):
